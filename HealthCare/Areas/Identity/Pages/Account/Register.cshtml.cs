...
namespace HealthCare.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                var user = new ApplicationUser { UserName = Input.Email, Email = Input.Email, Role = Roles.Athlete, CoachId = "0000000000" };
                ...
                if (result.Succeeded)
                {
                    ...
                    var roleResult = await _userManager.AddToRoleAsync(user, Roles.Athlete);
                    if (!roleResult.Succeeded)
                    {
                        ModelState.AddModelError(string.Empty, "An error occurred while assigning role to user.");
                        return Page();
                    }
